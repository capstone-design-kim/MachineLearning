...
try:
    with connection.cursor() as cursor:
        ...
        profile_sql = f"SELECT id, collaboration_count, my_time, desired_time, rate FROM profile WHERE id = '{user_id}'" #입력으로 받은 버튼 누른사람 user_id를 기준으로 가져옴
        cursor.execute(profile_sql)
        profile_row = cursor.fetchone()  # 해당 유저의 정보를 가져옴. 한 튜플만 가져옴.
        # 가져온 정보 : profile테이블 id, 상대에게 원하는 스택, 내가 투자가능한 시간, 상대에게 바라는 투자가능한 시간, 평점
          
        # 다른 유저들의 프로필 정보 저장할 리스트 초기화
        other_users_profiles = []

        if profile_row:     #profile_row가 존재하면, 즉 문제없이 버튼 누른 사람 user_id조회가 된다면
            # 참여 데이터 가져오기
            participation_sql = f"""
            ...
            # 예상 : (user_id, 스택 리스트) 튜플 여러개 리스트로 반환. 맵인가? print찍어봐야 할 것 같음
            # 여기까지 봤을 때, 입력에 contest_id도 받아야함.
            
            # 스택 리스트 조합
            stack_lists = [participation['stack_list'] for participation in participation_rows]
            combined_stack_list = ', '.join(stack_lists) #버튼 누른 사람의 원하는 스택 정보 한 문자열로 묶음(리스트). 

            # 프로필 정보 저장 (id, 원하는 스택, 원하는 경력 횟수, 원하는 투자시간, 내 평점)
            profile_info = {
                'id': profile_row['id'],    # profile테이블 id
                'stack_list': combined_stack_list,  # 합쳐진 한 문자열로 된 상대에게 원하는 스택
                'collaboration_count': profile_row['collaboration_count'], # 상대에게 원하는 협업프로젝트 경험횟수
                'desired_time': profile_row['desired_time'], # 상대에게 바라는 투자가능시간
                'rate': profile_row['rate'] # 내 평점
            }   # 딕셔너리로 변환
            print("Current User Profile:")
            print('누른 사람의 profile info : ', profile_info)
            # 여기까지 버튼 누른 사람의 정보 저장하고 출력. 
            
            # ================================================================================= #
            # 다른 유저들의 프로필 정보 가져오기 (id, 본인 스택, 경력횟수, 투자가능시간 ,별점)
            # 내가 아닌 다른 모든 사람에 대해 
            # profile테이블 id
            # (WHERE절 조건 !=) 중복 없이 stack_list 쉼표(,)로 구분해 연결한 문자열. 다른사람들 user_id를 굳이 알아낼 필요 없음.
            # career테이블 튜플수 (협업프로젝트 경험 횟수가 될 거임.)
            # 투자가능한 시간
            # 평점
            # 정보 가져옴.
            ...
            # fetchall()시 한 사람의 정보가 튜플로 묶여 반환 튜플의 리스트로 반환됨.
            ...
            for row in other_users_profile_rows:
                ...
                # 딕셔너리로 바꿔 딕셔너리 리스트로 변환.

            # 결과 출력
            print("\nOther Users Profiles:")
            df = pd.DataFrame(other_users_profiles)
            print(df) # 딕셔너리 리스트를 DataFrame으로 변환하면 어떻게되지...? 실행해봐야할듯
...
# Json 파일 만들기
json_path = '{0}_{1}.json'.format(contest_id, user_id)
